...
def sweep_ps_perc(cont_yaml,ps_perc=np.linspace(.75,1,num=8,endpoint=True).tolist(),group=2):
    # load default params          
    weis_dir            = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
    control_param_yaml  = os.path.join(weis_dir,'examples/OpenFAST_models/CT15MW-spar/ServoData/IEA15MW-CT-spar.yaml')
    inps                = yaml.safe_load(open(control_param_yaml))
    ...
    # add control gains to case_list
    case_inputs[('meta','ps_perc')]          = {'vals': ps_perc, 'group': group}
    case_inputs[('DISCON_in', 'PS_BldPitchMin')] = {'vals': m_ps, 'group': group}
